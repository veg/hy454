...
if ( Abs ( _recordsFound ) ) {
	...
	if ( dr_only ) { /*drug resistant mutations only*/
		...
		_recordsFound = {};
		DoSQL ( ANALYSIS_DB_ID, "SELECT SITE,SITE_HXB2,COVERAGE,CONSENSUS FROM " + MU_RATE_TABLE + ";", "return _matchRecordsByField(1);" );
		_siteInfo = _recordsFound;
		...
		for ( _k = 0; _k < Abs ( _siteInfo ); _k = _k + 1 ) {
			...
			for ( _l = 0; _l < numRates; _l = _l + 1 ) {
				SQLString = "";
				SQLString * 128;
				SQLString * ( "INSERT INTO " + NEB_TABLE + " VALUES ('" + (_siteInfo[_k])["SITE"] + "', '" + (_siteInfo[_k])["SITE_HXB2"] + "', '" + (_siteInfo[_k])["COVERAGE"] + "', '" + (_siteInfo[_k])["CONSENSUS"] + "', '" +  _l + "', '" + _rateWeightM[_l][0] + "', '" + _rateWeightM[_l][1] + "', '" + ((Exp(binomialP ( _rateWeightM[_l][0], total, choose ))*(_rateWeightM[_l][1]))/denominator) + "');" );
				SQLString * 0;
				DoSQL ( ANALYSIS_DB_ID, SQLString, "" );  
			}
		}
